...
bikerentals <- bikeshare%>%
  mutate(daytype = ifelse(workingday==0, "workingday", "weekend/holiday"))%>%
  group_by(hr, daytype)%>%
  summarize(avg_rentals = mean(total))
```
...
**Caption:** Based on the line graphs, the maximum number of average bike rentals on non-working days is higher than the maximum number of average bike rentals on working days. On non-working days, there are two extreme peaks when the average number of bike rentals are at their greatest, while on working days, the hour at which the average number of biking rentals is highest is the 13th hour, which would be 1:00 PM. For both working and non-working days, the average number of bike rentals start off low between the 0th and 5th hour and decline between the 17th and 23rd hour. The key takeaway is that on non-working days, the greatest number of average bike rentals occurs twice a day with drastic rises and declines, while on working days, the average number of bike rentals is more gradual, with the greatest average occurring in the early afternoon.
...
ggplot(bikerentals) + geom_col(aes(x=weathersit, y=avg_rentals), col="black", fill='darkgray') + labs(x="Weather Situation Code", y= "Average Number of Bike Rentals", title="Average Bike Rentals at 9 AM Based on Weather Situation") + facet_wrap(~daytype)
```

**Caption:** Based on the barplot, the average number of bike rentals were higher on non-working days regardless of the weather situation. There were no bike rentals on either type of day at the 9 AM hour in weather situation 4 (heavy rain + ice pallets + thunderstorm + mist, snow + fog). The average number of bike rentals at 9 AM on a non-working day were highest and almost about the same in weather situation numbers 1 (clear, few clouds, partly cloudy) and 2 (mist+cloudy, mist+broken clouds, mist+few clouds, mist). Meanwhile, the average number of bike rentals at 9 AM on working days were highest in weather situation 1, and decreases with each increase in weather situation code. The average number of bike rentals on a non-working day in weather situation 3 (light snow, light rain + thunderstorm + scattered clouds, light rain + scattered clouds) was greater than that of a working day. The key takeaway is that the average number of bike rentals differs according to the weather situation more on working days than on non-working days. 
...
ggplot(capmetro) + geom_point(aes(x=temperature, y=boarding, color=weekend), cex=.7) + labs(title="Relationship Between Bus Boardings and Temperature", x="Temperature (F)", y="Number of Bus Boardings") + facet_wrap(~hour_of_day, nrow=4)
...
top10 <- billboard%>%
  ...
  summarize(count=sum(weeks_on_chart))%>%
  arrange(desc(count))%>%
  head(10)
```
